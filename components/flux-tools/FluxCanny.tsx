...
interface CannyGeneratorProps {
    dictionary: any;
    locale: string;
    config: CannyConfig;
}

interface CannyState {
...
const CannyGenerator = ({dictionary, locale, config}: CannyGeneratorProps) => {
    const [state, setState] = useState<CannyState>({
        sourceImage: null,
    ...
    const updateState = (updates: Partial<CannyState>) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/flux-tools/canny', {
                method: 'POST',
                credentials: 'include',
                body: formData,
                headers: {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            link.download = `flux-canny-${timestamp}.${state.outputFormat}`;
            ...
            <div className="container mx-auto z-10 max-w-7xl flex flex-col">
                ...
                <div className="pb-6 text-center">
                    <h1 className="text-4xl md:text-5xl font-bold animate-fade-in-down mb-3">
                        {dictionary.fluxTools.canny.pageTitle}
                    </h1>
                    <p className="text-lg md:text-xl text-white/80">
                        {dictionary.fluxTools.canny.description}
                    </p>
                </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-4">
                            ...
                            <div className="space-y-2 col-span-2">
                                <label className="text-sm font-medium text-white/90">
                                    {dictionary.fluxTools.canny.promptLabel}
                                </label>
                                <textarea
                                    value={state.prompt}
                                    onChange={(e) => updateState({prompt: e.target.value})}
                                    placeholder={dictionary.fluxTools.canny.promptPlaceholder}
                                    rows={3}
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-white/90">
                                    {dictionary.fluxTools.canny.guidanceLabel}
                                </label>
                                <input
                                    type="number"
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-white/90">
                                    {dictionary.fluxTools.canny.inferenceStepsLabel}
                                </label>
                                <input
                                    type="number"
                                    value={state.steps}
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-white/90">
                                    {dictionary.fluxTools.canny.safetyToleranceLabel}
                                </label>
                                <input
                                    type="number"
                            ...
                            <div className="space-y-2 col-span-2">
                                <label className="flex items-center space-x-2 text-sm font-medium text-white/90">
                                    ...
                                    />
                                    <span>{dictionary.fluxTools.canny.promptUpsamplingLabel}</span>
                                </label>
                            </div>
                        </div>
