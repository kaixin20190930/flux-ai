...
export function useAuthStatus() {
  const [isAuthenticated, setIsAuthenticated] = useState(
    unifiedAuthManager.getState().isAuthenticated
  );
  const [loading, setLoading] = useState(
    unifiedAuthManager.getState().loading
  );
...
export function useCurrentUser() {
  const [user, setUser] = useState<User | null>(
    unifiedAuthManager.getState().user
  );
  const [loading, setLoading] = useState(
    unifiedAuthManager.getState().loading
  );
...
export function useUserPoints() {
  const [userPoints, setUserPoints] = useState<number | null>(
    unifiedAuthManager.getState().userPoints
  );
  const [loading, setLoading] = useState(
    unifiedAuthManager.getState().loading
  );
...
export function useAuthError() {
  const [error, setError] = useState(unifiedAuthManager.getState().error);
...
export function useAuthDebug() {
  ...
  const logState = useCallback(() => {
    if (process.env.NODE_ENV === 'development') {
      ...
      console.log('Current state:', unifiedAuthManager.getState());
